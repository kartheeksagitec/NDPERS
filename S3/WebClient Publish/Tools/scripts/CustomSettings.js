app.controller('customSettingsController', ["$scope", "$rootScope", "$timeout", function ($scope, $rootScope, $timeout) {
    ...
    $scope.updateModelForBaseAppModel = function (updateParentName) {
        if ($scope.IsBaseAppFile && $scope.customsettingsfiledata && $scope.customsettingsfiledata.BaseAppModel) {
            $scope.$evalAsync(function () {
                updateModelForBaseAppModel($scope.customsettingsfiledata, $scope.customsettingsfiledata.BaseAppModel, updateParentName);
            });
        }
    };
    ...
    return {
        ...
        link: function (scope, element, attributes) {
            ...
            scope.updateModelForBaseAppModel = function (updateParentName) {
                ...
                if (controllerScope && controllerScope.updateModelForBaseAppModel) {
                    controllerScope.updateModelForBaseAppModel();
                }
            };
            scope.revertToBaseAppModel = function (model) {
                var controllerScope = getCurrentFileScope();
                if (controllerScope && controllerScope.revertToBaseAppModel) {
                    controllerScope.revertToBaseAppModel(model);
                }
            };
