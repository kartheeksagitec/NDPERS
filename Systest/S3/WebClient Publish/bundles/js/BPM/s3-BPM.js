app.controller('BPMNController', ["$scope", "$rootScope", "$filter", "$getQueryparam", "$getMultiIndexofArray", "$getEnitityRule", "$getEnitityXmlMethods", "$cssFunc", "$EntityIntellisenseFactory", "$NavigateToFileService", "$getEnitityObjectMethods", "hubcontext", function ($scope, $rootScope, $filter, $getQueryparam, $getMultiIndexofArray, $getEnitityRule, $getEnitityXmlMethods, $cssFunc, $EntityIntellisenseFactory, $NavigateToFileService, $getEnitityObjectMethods, hubcontext) {
    ...
    $scope.initVariables = function () {
        ...
        $scope.dvtypes = ['Query', 'Constant', 'CodeGroup'];
    ...
    $scope.receiveBpmnModel = function (data) {
        $scope.$apply(function () {
            ...
            $scope.rules = $scope.BPMNModel.objExtraData.Rules;
        });
    };
    ...
    $scope.changeMapType = function (mapvar) {
        if (mapvar.dictAttributes.dataType == 'Object') {
            ...
            $rootScope.EditPropertyValue(mapvar.dictAttributes.sfwCodeID, mapvar.dictAttributes, "sfwCodeID", "");
            $rootScope.UndRedoBulkOp("End");
        }
        else {
            $rootScope.EditPropertyValue(mapvar.dictAttributes.sfwEntity, mapvar.dictAttributes, "sfwEntity", "");
        }
    };
    $scope.changeMapdvType = function (mapvar) {
        if (mapvar.dictAttributes.sfwDefaultValueType == 'Query') {
            $rootScope.UndRedoBulkOp("Start");
            $rootScope.EditPropertyValue(mapvar.dictAttributes.sfwDefaultValue, mapvar.dictAttributes, "sfwDefaultValue", "");
            $rootScope.EditPropertyValue(mapvar.dictAttributes.sfwCodeID, mapvar.dictAttributes, "sfwCodeID", "");
            $rootScope.UndRedoBulkOp("End");
        }
        else if (mapvar.dictAttributes.sfwDefaultValueType === 'CodeGroup') {
            $rootScope.UndRedoBulkOp("Start");
            $rootScope.EditPropertyValue(mapvar.dictAttributes.sfwDefaultValue, mapvar.dictAttributes, "sfwDefaultValue", "");
            $rootScope.EditPropertyValue(mapvar.dictAttributes.sfwQueryID, mapvar.dictAttributes, "sfwQueryID", "");
            $rootScope.EditPropertyValue(mapvar.dictAttributes.sfwQueryParameters, mapvar.dictAttributes, "sfwQueryParameters", "");
        ...
        if (redeploy) {
            ...
            newScope.onOKClick = function () {
                ...
                $.connection.hubBPMN.server.deployMap($rootScope.currentopenfile.file.FilePath, newScope.selectedCaseID).done(function (data) {
                    ...
                    $scope.showValidationResult(data, newScope.selectedCaseID);
                    if (!(data && data.length)) {
            ...
            $.connection.hubBPMN.server.deployMap($rootScope.currentopenfile.file.FilePath, 0).done(function (data) {
                ...
                $scope.showValidationResult(data);
                if (!(data && data.length)) {
                    if ($scope.ActiveTabForBPM == 'Versions') {
                        $scope.loadVersions();
                    }
    ...
    $scope.showValidationResult = function (validationResult, caseId) {
        if (validationResult && validationResult.length) {
            var newScope = $scope.$new();
            newScope.errorList = validationResult;
            newScope.templateName = "BPM/views/BPMValidationErrors.html";
            newScope.dialogOptions = { height: 245, width: 600, showclose: true };
            newScope.onOKClick = function () {
                newScope.closeDialog();
            };
            newScope.closeDialog = function () {
            ...
            newScope.dialog = $rootScope.showDialog(newScope, "Validation Errors", newScope.templateName, newScope.dialogOptions);
        }
        else {
            if (caseId) {
                toastr.success(string.format("Map for Case ID '{0}' re-deployed successfully.", caseId));
            }
            else {
                toastr.success("Map deployed successfully.");
            }
        }
    }
    ...
    $scope.showMapDetails = function () {
        ...
        newScope.Init = function () {
            newScope.BPMNModel = $scope.BPMNModel;
        ...
        newScope.onOKClick = function () {
            ...
            newScope.$broadcast("UpdateBpmTerminate", null);
            $rootScope.UndRedoBulkOp("End");
            //close the dialog
            newScope.closeDialog();
        };
        ...
        newScope.dialog = $rootScope.showDialog(newScope, "Details", "BPM/views/BpmDetailsTemplate.html", { width: 700, height: 350 });
        newScope.Init();
    };
    ...
    $scope.validateFileData = function () {
        $.connection.hubBPMN.server.validateMap().done(function (data) {
            if (data && data.length) {
                $scope.showValidationResult(data);
            }
            else {
                toastr.success("Map validated successfully.");
            }
        });
    }
    ...
    $scope.createRule = function (rule) {
        ...
        if (scope) {
            scope.addRules("LogicalRule", $scope, rule.Entity, rule.RuleName.substr(3), false)
        }
    }
}]);
...
function bpmUsertaskProcessMapPropertiesController($rootScope) {
    this.initialize = function () {
        ...
        if (this.mapvariablesmodel) {
            ...
            this.intTypeVariables = this.mapvariablesmodel.Elements.filter(function (itm) { return itm.dictAttributes.dataType === "int"; }).map(function (itm) { return itm.dictAttributes.id; });
        }
    };
    this.onChangeSLAFromVariable = function () {
        $rootScope.UndRedoBulkOp("Start");
        $rootScope.EditPropertyValue(this.model.dictAttributes.Days, this.model.dictAttributes, "Days", "");
        $rootScope.EditPropertyValue(this.model.dictAttributes.Hours, this.model.dictAttributes, "Hours", "");
        $rootScope.EditPropertyValue(this.model.dictAttributes.Minutes, this.model.dictAttributes, "Minutes", "");
        $rootScope.UndRedoBulkOp("End");
    }
    this.initialize();
}
...
app.component('bpmUsertaskShapeMapProperties', {
    ...
    controller: ["$rootScope", bpmUsertaskProcessMapPropertiesController]
});
        ...
        link: function (scope, element, attrs) {
            ...
            scope.UserRoleTextChanged = function (event) {
                ...
                if (event.ctrlKey && event.keyCode == $.ui.keyCode.SPACE && $(input).data('ui-autocomplete')) {
                    ...
                    setSingleLevelAutoComplete(input, scope.intellienseList, scope, "CodeID", "CodeIDDescription");
                }
            };
            scope.showIntellisenseList = function (event) {
                var input = $(event.target).prevAll("input[type='text']");
                setSingleLevelAutoComplete(input, scope.lstuserroles, scope, "CodeID", "CodeIDDescription");
                if ($(input).data('ui-autocomplete')) {
