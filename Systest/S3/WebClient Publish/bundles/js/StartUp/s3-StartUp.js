...
app.factory('ConfigurationFactory', ["$rootScope", function ($rootScope) {
    var lstProjectName = [];
    var objLastProject = {};
    var projectTypeProvider = {};
    return {
        ...
        setConfigurationObject: function (lstprojectName, lastProjectDetails) {
            ...
            this.setCurrentProjectTypeToRoot();
        },
        setProjectNames: function (lstprojectName) {
            lstProjectName = lstprojectName;
        },
        setProjectTypeProvider: function (aobjProjectTypeProvider) {
            projectTypeProvider = aobjProjectTypeProvider;
            this.setCurrentProjectTypeToRoot();
        },
        getProjectTypeProvider: function () {
            return projectTypeProvider;
        },
        getCurrentProjectAppBaseFolderName: function () {
            var projectType = this.getCurrentProjectType();
            if (projectType) {
                return projectType.BasePath;
            }
            return "";

        },
        getCurrentProjectType: function () {
            if (objLastProject && projectTypeProvider && projectTypeProvider.ProjectTypeList) {
                var projectType = projectTypeProvider.ProjectTypeList.filter(function (itm) { return itm.Type === objLastProject.ProjectType; })[0];
                return projectType;
            }
        },
        setCurrentProjectTypeToRoot: function () {
            var projectType = this.getCurrentProjectType();
            if (projectType) {
                $rootScope.currentProjectType = projectType;
            }
        }
    };
}]);
