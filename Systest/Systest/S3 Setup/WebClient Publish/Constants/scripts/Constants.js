app.controller("ConstantsController", ["$scope", "$filter", "$rootScope", "$timeout", "$resourceFactory", "CONSTANTS", "$ValidationService", "$Errors", function ($scope, $filter, $rootScope, $timeout, constantFactory, CONST, $ValidationService, $Errors) {
    $scope.Groups = [];
    ...
    //hubMain.server.loadConstants(false);
    //hubMain.server.getModel($rootScope.currentopenfile.file.FilePath, $rootScope.currentopenfile.file.FileType);

    //$scope.receiveRuleConstantsModel = function (data) {
    //    $rootScope.IsLoading = false;
    //    $scope.constantfilemodel = JSON.parse(data);
    ...
    //var clonedModel = JSON.parse(data);
    ...
    $scope.LoadEntityQueries = function () {
        $.connection.hubConstants.server.getEntityQueriesList().done(function (data) {
            $scope.$apply(function () {
                $rootScope.IsLoading = false;

                $scope.EntityQueryCollection = data;
                $scope.IsEntityQueriesLoaded = true;
    ...
    $scope.LoadParentForObject = function (objParent, item) {
        ...
        if (objParent != undefined) {
            ...
            if (item.Name == "constant") {
                ...
                if (item.ParentGroupType == "Query" || item.ParentGroupType == "Files" || item.ParentGroupType == "EntityQueries") {
    ...
    $scope.ResetGroupQueryDetail = function () {
                        ...
                        $scope.$apply(function () {
                            if (data) {
                                ...
                                angular.forEach(lst, function (item) {
                                    //$scope.SelectedConstant.Elements.push(item);
    ...
    $scope.menuOptionsForConstant = [
        ['Add Groups', function ($itemScope) {
            ...
            $scope.SelectedConstant = $itemScope.itm;
            toggleSelectionOfConstants();
