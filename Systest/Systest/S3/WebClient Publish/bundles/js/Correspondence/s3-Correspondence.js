app.controller("CorrespondenceController", ["$scope", "$http", "$rootScope", "ngDialog", "$EntityIntellisenseFactory", "$NavigateToFileService", "CONSTANTS", "$ValidationService", "$Errors", "$filter", "$timeout", "$GetEntityFieldObjectService", "$Entityintellisenseservice", "ConfigurationFactory", function ($scope, $http, $rootScope, ngDialog, $EntityIntellisenseFactory, $NavigateToFileService, CONST, $ValidationService, $Errors, $filter, $timeout, $GetEntityFieldObjectService, $Entityintellisenseservice, ConfigurationFactory) {
            ...
            $.connection.hubForm.server.getGlobleParameters().done(function (data) {
                if (data) {
                    $scope.$apply(function () {
                        $scope.ilstGlobalParameters = data.Elements.map(function (gp) { return gp.dictAttributes.ID; });
                    });
                }
            });
        ...
        if (path != null && path != "form" && path != "") {
            ...
            if (items.length > 0) {
                ...
                if (items[0].Name == "sfwRuleBookmarks") {
                    ...
                    idSelectedDiv = "correspondence-RuleBookMark-tab-content";
                }
                if (items[0].Name === "sfwHyperlinkBookmarks") {
                    $scope.selectRuleTab();
                    if (items[items.length - 1].Name === "sfwHyperlinkBookmark") {
                        $scope.selectHyperlinkBookmark(items[items.length - 1]);
                    }
                    idSelectedDiv = "correspondence-HyperlinkBookmarks-tab-content";
                }
        ...
        if ($scope.selectedDesignSource == false) {
            ...
            if ($scope.objCorrespondenceDetails != null && $scope.objCorrespondenceDetails != undefined) {
                ...
                if (objreturn1 != "") {
                    ...
                    else if ($scope.selectedRuleBookMark && $scope.SelectedCorrespondenceTab == 'VisibleRule') {
                        selectedCorrespondenceItem = $scope.selectedRuleBookMark;
                    }
                    else if ($scope.selectedHyperlinkBookmark && $scope.SelectedCorrespondenceTab == 'Hyperlink') {
                        selectedCorrespondenceItem = $scope.selectedHyperlinkBookmark;
                    }
    ...
    $scope.receiveCorrespondencemodel = function (data) {
        $scope.$apply(function () {
            ...
            for (var i = 0; i < $scope.objCorrespondenceDetails.Elements.length; i++) {
                ...
                else if ($scope.objCorrespondenceDetails.Elements[i].Name == 'sfwHyperlinkBookmarks') {
                    $scope.objHyperlinkBookmarks = $scope.objCorrespondenceDetails.Elements[i];
                    $scope.setParentVM($scope.objCorrespondenceDetails.Elements[i]);
                }
            ...
            if (!($scope.objCorrespondenceDetails.dictAttributes.sfwRemoveBookmarksAfterGeneration && $scope.objCorrespondenceDetails.dictAttributes.sfwRemoveBookmarksAfterGeneration.toLowerCase() === "false")) {
                $scope.objCorrespondenceDetails.dictAttributes.sfwRemoveBookmarksAfterGeneration = "";
            }
            ...
            if (!$scope.objHyperlinkBookmarks) {
                $scope.objHyperlinkBookmarks = $scope.CreateNewObject("sfwHyperlinkBookmarks", $scope.objCorrespondenceDetails);
            }
    ...
    $scope.selectHyperlinkBookmark = function (obj) {
        $scope.selectedHyperlinkBookmark = obj;
    };
    ...
    $scope.onDetailClick = function () {
        ...
        newScope.Init = function () {
            ...
            newScope.removeBookmarksAfterGeneration = !newScope.objCorrespondenceDetails.dictAttributes.sfwRemoveBookmarksAfterGeneration;
            $scope.PopulateCorrespondenceTemplate();
        ...
        newScope.onRemoveBookmarksAfterGenerationChange = function () {
            if (newScope.removeBookmarksAfterGeneration) {
                newScope.objCorrespondenceDetails.dictAttributes.sfwRemoveBookmarksAfterGeneration = "";
            }
            else {
                newScope.objCorrespondenceDetails.dictAttributes.sfwRemoveBookmarksAfterGeneration = "False";
            }
        };
    ...
    $scope.receieveSyncronizeBookMarkData = function (data) {
        ...
        if ($scope.objHyperlinkBookmarks.Elements.length == 0) {
            $scope.selectedHyperlinkBookmark = undefined;
        } else if ($scope.objHyperlinkBookmarks.Elements.length > 0) {
            $scope.selectedHyperlinkBookmark = $scope.objHyperlinkBookmarks.Elements[0];
        }
    ...
    $scope.synchronizeBookmarksValues = function (alBookmarks) {
        for (var i = 0; i < alBookmarks.length; i++) {
            ...
            if (!$scope.FindBookmark(corBookmark)) {
                ...
                else if ($scope.IsHyperlinkBookmark(corBookmark)) {
                    var hyperlinkBookmark = { Name: 'sfwHyperlinkBookmark', Value: '', dictAttributes: { ID: corBookmark.Name }, Elements: [] };
                    $rootScope.PushItem(hyperlinkBookmark, $scope.objHyperlinkBookmarks.Elements);
                }
        ...
        //Deleting old hyperlink bookmarks
        if ($scope.objHyperlinkBookmarks) {
            for (var i = $scope.objHyperlinkBookmarks.Elements.length - 1; i >= 0; i--) {
                var hyperlinkBookmark = $scope.objHyperlinkBookmarks.Elements[i];
                if (!alBookmarks.some(function (bk) { return bk.Name === hyperlinkBookmark.dictAttributes.ID; })) {
                    $rootScope.DeleteItem(hyperlinkBookmark, $scope.objHyperlinkBookmarks.Elements);
                }
            }
        }
    };
    ...
    $scope.FindBookmark = function (corBookmark) {
        ...
        if ($scope.IsTableBookMark(corBookmark)) {
            ...
            (corBookmark.Type === "QueryConditionalBlock") || corBookmark.Type === "QueryCheckBoxControl") {
        ...
        else if (corBookmark.Type === "HyperLink") {
            if ($scope.objHyperlinkBookmarks) {
                alBookmarks = $scope.objHyperlinkBookmarks.Elements;
            }
        }
        else {
            if ($scope.objFieldBookMarks) {
    ...
    $scope.IsFieldBookMark = function (corBookmark) {
        if (($scope.IsTableBookMark(corBookmark)) || ($scope.IsQueryBookMark(corBookmark)) || ($scope.IsTemplateBookMark(corBookmark)) || ($scope.IsRuleBookMark(corBookmark)) || ($scope.IsRepeaterBookMark(corBookmark)) || ($scope.IsHyperlinkBookmark(corBookmark))) {
            return false;
        }
    ...
    $scope.IsHyperlinkBookmark = function (corBookmark) {
        return corBookmark.Type === "HyperLink";
    };
    ...
    $scope.selectHyperlinkTab = function () {
        $scope.SelectedCorrespondenceTab = 'Hyperlink';
        $scope.ActiveTabForCorr = 'Entity';
        if ($scope.objHyperlinkBookmarks && $scope.objHyperlinkBookmarks.Elements.length > 0) {
            $scope.selectHyperlinkBookmark($scope.objHyperlinkBookmarks.Elements[0]);
    ...
    $scope.LoadBookmarks = function (bookmarks, obj, fields) {
        $scope.$evalAsync(function () {
            if (obj.dictAttributes.sfwIsObjectBased !== 'True') {
                var simpleBookmarks = bookmarks.filter(function (x) { return x.Type === BookmarkType.Simple || x.Type === BookmarkType.ConditionalBlock || x.Type === BookmarkType.CheckBoxControl });
            ...
            if ($scope.objHyperlinkBookmarks && obj.dictAttributes.sfwIsObjectBased !== 'True') {
                var elements = $scope.objHyperlinkBookmarks.Elements;
                $scope.CreateTestFields(elements, "Hyperlink", obj, fields);
            }
    ...
    $scope.CreateTestFields = function (elements, bookMarkType, obj, oldFields) {
        angular.forEach(elements, function (item) {
            ...
            if (oldFields) {
                ...
                if (oldBookMark && oldBookMark.length > 0) {
                    field.dictAttributes.Value = oldBookMark[0].dictAttributes.Value;
                    if (item.Name === "sfwHyperlinkBookmark") {
                        field.dictAttributes.sfwDisplayText = oldBookMark[0].dictAttributes.sfwDisplayText;
                    }
