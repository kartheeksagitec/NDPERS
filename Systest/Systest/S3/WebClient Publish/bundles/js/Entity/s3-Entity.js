...
app.controller("EntityController", ["$scope", "$http", "$rootScope", "hubcontext", "$FormatQueryFactory", "$resourceFactory", "$EntityIntellisenseFactory", "$NavigateToFileService", "$filter", "CONSTANTS", "$ValidationService", "$Errors", "$timeout", "$interval", "ConfigurationFactory", "$GetEntityFieldObjectService", "$compile", "$searchFindReferences", "$Entityintellisenseservice", function ($scope, $http, $rootScope, hubcontext, $FormatQueryFactory, $resourceFactory, $EntityIntellisenseFactory, $NavigateToFileService, $filter, CONST, $ValidationService, $Errors, $timeout, $interval, ConfigurationFactory, $GetEntityFieldObjectService, $compile, $searchFindReferences, $Entityintellisenseservice) {
    $scope.initialize = function () {
        ...
        $scope.lstLoadTypes = [];
        $scope.lstSortOrder = [{ key: "asc", value: "Ascending" }, { key: "desc", value: "Descending" }];
        ...
        $scope.hasAppJsonDataColumn = false;
        //#endregion

        //DBTypes
        var Projectdetails = ConfigurationFactory.getLastProjectDetails();
        var strQueryTypes = Projectdetails.QueryTypes;
        $scope.IsBaseAppFile = $scope.currentfile.IsBaseAppFile;
        $scope.ProjectType = Projectdetails.ProjectType;
        if (!strQueryTypes) {
        ...
        $scope.objnewJsonAttribute = getNewBaseModel("attribute", { ID: '', sfwDataType: 'string', sfwDataFormat: '', sfwPrivate: false, sfwValue: '', sfwEnumerator: '', Text: '', sfwType: "AppJsonData" });
    ...
    $scope.receiveentitymodel = function (data) {
        ...
        $scope.$evalAsync(function () {
            ...
            if ($scope.objEntity.dictAttributes.sfwObjectID != undefined && $scope.objEntity.dictAttributes.sfwObjectID != "") {
                ...
                $scope.getPartialFileData();
            }

            if (!$scope.lstBusinessObject) {
                hubMain.server.getLstBusinessObject(true).done(function (data) {
                    $scope.$evalAsync(function () {
            ...
            $scope.checkHasAppJsonColumn();
        });

    };
    $scope.getPartialFileData = function () {
    ...
    $scope.onChangeDeletePropertyId = function (obj) {
        ...
        $ValidationService.checkDuplicateId(obj, $scope.objDelete, [], true, ["delete", "property"], [], $rootScope.iblnShowEntityDuplicateIdAsWarning, true);
    };
    ...
    $scope.canRemoveDeleteProperty = function () {
        ...
        if (selectedItems && selectedItems.length > 0 && !($scope.IsBaseAppFile && selectedItems[0].IsBaseAppModel)) {
            return true;
        }
        return false;
    };
    ...
    $scope.canMoveDeletePropertyUp = function () {
        ...
        var selectedItems = $scope.objDelete.Elements.filter(function (x) { return x.isSelected == true; });
        if (selectedItems && selectedItems.length > 0) {
            var delProperties = $scope.objDelete.Elements.filter(function (x) { return x.Name == "property"; });
            var idx = delProperties.indexOf(selectedItems[0]);
            if (idx > 0 && !($scope.IsBaseAppFile && selectedItems[0].IsBaseAppModel)) {
                retValue = true;
                if (idx > 0 && delProperties[idx - 1].IsBaseAppModel) {
                    retValue = false;
                }
            }
    ...
    $scope.canMoveDeletePropertyDown = function () {
        ...
        if (selectedItems && selectedItems.length > 0) {
            return $scope.objDelete.Elements.filter(function (x) { return x.Name == "property"; }).indexOf(selectedItems[0]) < $scope.objDelete.Elements.filter(function (x) { return x.Name == "property"; }).length - 1 &&
                !($scope.IsBaseAppFile && selectedItems[0].IsBaseAppModel);
        }
        return false;
    };
    ...
    $scope.canDeleteCSProperty = function () {
        if ($scope.PartialFileData != undefined) {
            return $scope.PartialFileData.some(function (x) { return x.isSelected && !($scope.IsBaseAppFile && x.IsBaseAppProperty); });
        }
    };
                        ...
                        if (item.Name == "queries") {
                            ...
                            if ($scope.IsDirtyPartialFile && $scope.BusObjPartialFileName && $scope.BusObjPartialFileName.trim().length > 0) {
                                ...
                                if ($scope.isDirty) {
                                    ...
                                    if (pageLayoutScope && pageLayoutScope.onsavefileclick) {
                                        $timeout(function () {
                                            $scope.isSilentSave = true;
                                            pageLayoutScope.onsavefileclick($rootScope.currentopenfile);
                                        });
                                    }
                                }
                            ...
                            angular.forEach(item.Elements, function (method) {
                                var objXmlMethod = {
                                    ID: method.dictAttributes.ID, Type: "XMLMethod", MethodType: method.dictAttributes.sfwMethodType, Mode: method.dictAttributes.sfwMode, Items: [], Parameters: []
                                };
    ...
    $scope.AddMethod = function (obj, flag) {
        ...
        if (flag === "Add") {
            ...
            if (obj) {
                newMethodScope.obj = JSON.parse(JSON.stringify(obj));
                newMethodScope.obj.dictAttributes.ID = "";
                if (newMethodScope.obj.IsBaseAppModel) {
                    newMethodScope.obj.IsBaseAppModel = undefined;
                    newMethodScope.obj.Elements.forEach(function (x) { x.IsBaseAppModel = undefined; x.$$hashKey = undefined; });
                }
                if (newMethodScope.obj.$$hashKey) {
                    newMethodScope.obj.$$hashKey = undefined;
                }
            }
            else {
                newMethodScope.obj = {
        ...
        newMethodScope.setXmlMethodNavigationParameter = function (selectedXmlMethod) {
            $scope.xmlMethodParameters = selectedXmlMethod.Elements.filter(function (x) { return x.Name == "parameter"; }).map(function (y) {
                return {
                    ...
                    IsDisabled: y.IsBaseAppModel || y.BaseAppModel
                };
            });
            var newScope = $scope.$new(true);
            if ($scope.IsBaseAppFile && selectedXmlMethod.IsBaseAppModel) {
                newScope.IsDisableFields = true;
            }
            else {
                newScope.IsDisableFields = false;
            }
        ...
        newMethodScope.getForignKeysForselectedObject = function (objItem) {
            if (objItem.dictAttributes.ID && objItem.dictAttributes.ID.trim() != "") {
                ...
                if (object) {
                    ...
                    objItem.relatedEntityId = object.Entity;
                }
        ...
        newMethodScope.resetLoadSource = function (objmethod) {
            if (objmethod) {
                ...
                objmethod.dictAttributes.sfwOrderBy = "";
                objmethod.dictAttributes.sfwSortOrder = "";
                ...
                if (selectedXmlMethod && busMethod) {
                    if (busMethod.dictAttributes.ID && busMethod.dictAttributes.ID.trim() != "") {
                        ...
                        if (object) {
                            BusinessObjectName = $scope.GetBusinessObject(object.DeclaringEntity);
                            EntityName = object.DeclaringEntity;
                            busMethod.relatedEntityId = object.Entity;
                        }
                        if (BusinessObjectName && $scope.blnShowCreateWithClass) {
    ...
    $scope.canDeleteMethod = function () {
        if ($scope.objMethods != undefined && $scope.objMethods.Elements != undefined) {
            return $scope.objMethods.Elements.some(function (x) {
                return x.selected && !($scope.IsBaseAppFile && x.IsBaseAppModel);
            });
        }
    };
    ...
    $scope.refreshColumns = function (showConfirmation = true) {
        if ($scope.checkDbAndDirty("refreshing entity columns")) {
            var result = true;
            if (showConfirmation) {
                result = confirm("Refreshing the entity will update and save the file.Do you want to continue?");
            }
            if (result) {
                $rootScope.ClearUndoRedoListByFileName($scope.currentfile.FileName);
                if ($scope.objEntity.dictAttributes.sfwTableName != undefined && $scope.objEntity.dictAttributes.sfwTableName != "") {
                    ...
                    $.connection.hubEntityModel.server.refreshEntityTable($rootScope.currentopenfile.file).done(function (data) {
                        ...
                        $rootScope.updateUserFilesForApp(objRefreshEntity.FileInfo);
                        $rootScope.IsLoading = false;
                    });
                }
            }
    ...
    $scope.checkDbAndDirty = function (operation) {
        var lblnReturnValue = true;
        ...
        if (mainPageScope && mainPageScope.strDBConnection && mainPageScope.strDBConnection === "DB : Not Connected.") {
            ...
            alert('Please save the file before ' + operation + '.');
            lblnReturnValue = false;
        }
    ...
    $scope.disableQueryDelete = function () {
        ...
        else if ($scope.IsBaseAppFile && $scope.selectedQuery.IsBaseAppModel) {
            return true;
        }
        else {
            return false;
    ...
    $scope.RulesMenuOptions = [
        ...
        ['Delete Rule', function ($itemScope) {
            ...
            if ($itemScope.element != undefined) {
                if ($itemScope.element.Name == "rule" && !$itemScope.element.BaseAppModel && !$itemScope.element.IsBaseAppModel) {
        ...
        ['Revert', function ($itemScope) {
            ...
            if ($itemScope.element != undefined) {
                $scope.SelectedRule = $itemScope.element;
                $scope.revertToBaseAppModel($scope.SelectedRule);
                ...
                if ($scope.SelectedRule.IsMessageSelected == "MessageID") {
                    $scope.populateMessageByMessageID($scope.SelectedRule.dictAttributes.sfwMessageId, '', true, false);
                }
                else {
                    $scope.populateMessageByMessageDescription(false);
                }
                ...
                if (Error) {
                    ...
                    if (newScope.selectedRule != undefined) {
                        ...
                        newScope.validateNewGroup = function (type) {
                            ...
                            else if (NewId.toLowerCase() === "none") {
                                newScope.groupErrorMessageForDisplay = "Error: ID cannot be 'None'.";
                                return true;
                            }
                            else {
                                var objParentRules = null;
    ...
    $scope.populateMessageByMessageID = function (messageID, MessageParameters, addinundoredo, updateForBaseAppModel = true) {
        ...
        if (updateForBaseAppModel) {
            $scope.updateModelForBaseAppModel();
        }
    };
    /* clearing params after changing Message Description */
    $scope.populateMessageByMessageDescription = function (updateForBaseAppModel = true) {
        $rootScope.EditPropertyValue($scope.SelectedRule.dictAttributes.sfwMessageParameters, $scope.SelectedRule.dictAttributes, "sfwMessageParameters", "");
        //if ($scope.SelectedRule.dictAttributes.sfwMessageId == "" && $scope.SelectedRule.dictAttributes.sfwMessage == "") {
        $ValidationService.validateIsEmptyMessageIdAndDescription($scope.SelectedRule, $scope.validationErrorList, $scope.SelectedRule.dictAttributes.sfwMessageId, $scope.SelectedRule.dictAttributes.sfwMessage);
        //}
    ...
    $scope.UpdateRuleParameter = function () {
    ...
    $scope.ErrorMenuOptionsForHardError = [
        ['Add Child Rule', function ($itemScope) {
            ...
            if ($itemScope.element != undefined && $itemScope.element.Name == "item" && !$itemScope.element.BaseAppModel && !$itemScope.element.IsBaseAppModel) {
        ...
        ['Delete', function ($itemScope) {
            ...
            if ($itemScope.element != undefined && !$itemScope.element.BaseAppModel && !$itemScope.element.IsBaseAppModel) {
                $scope.SelectedErrorClick($itemScope.element, null, "HardError");
                return true;
            }
        }
        ...
        ['Revert', function ($itemScope) {
            $scope.SelectedErrorRule = $itemScope.element;
            if ($scope.SelectedErrorRule != undefined) {
                $scope.revertToBaseAppModel($itemScope.element);
            }
        }, function ($itemScope) {
            if ($itemScope.element != undefined && $itemScope.element.BaseAppModel && !$itemScope.element.IsBaseAppModel) {
                $scope.SelectedErrorClick($itemScope.element, null, "HardError");
    ...
    var CutOrCopyQuery = function (param) {
        if (param == 'copy') {
            ...
            data.IsBaseAppModel = false;
            data.BaseAppModel = null;
    ...
    $scope.reverQuery = function () {
        if ($scope.selectedQuery.BaseAppModel) {
            $scope.revertToBaseAppModel($scope.selectedQuery);
        }
        else {
            for (var i = 0; i < $scope.selectedQuery.Elements.length; i++) {
                for (var j = 0; j < $scope.selectedQuery.Elements[i].Elements.length; j++) {
                    if ($scope.selectedQuery.Elements[i].Elements[j].BaseAppModel) {
                        $scope.revertToBaseAppModel($scope.selectedQuery.Elements[i].Elements[j]);
                    }
                }
            }
        }
        ...
        if (index > -1) {
            $scope.$evalAsync(function () {
                $scope.objQueries.Elements.splice(index, 1);
                $scope.objQueries.Elements.splice(index, 0, $scope.selectedQuery);
                $scope.selectQuery($scope.selectedQuery, 'entity-queries-section');
            });
        }
        $scope.$evalAsync(function () {
            $scope.showSelectedQueryInEditor();
    ...
    $scope.queryMenuOptions = [
        ['Rename', function ($itemScope) {
            ...
            return ($scope.objQueries != undefined && $scope.selectedQuery && !$scope.selectedQuery.BaseAppModel && !$scope.selectedQuery.IsBaseAppModel);
        }],
        ['Delete', function ($itemScope) {
        ...
        ['Revert', function ($itemScope) {
            ...
            if ($scope.objQueries != undefined && $scope.selectedQuery) {
                if ($scope.selectedQuery.BaseAppModel && !$scope.selectedQuery.IsBaseAppModel) {
                    return true;
                }
                else {
                    for (var i = 0; i < $scope.selectedQuery.Elements.length; i++) {
                        for (var j = 0; j < $scope.selectedQuery.Elements[i].Elements.length; j++) {
                            if ($scope.selectedQuery.Elements[i].Elements[j].BaseAppModel && !$scope.selectedQuery.Elements[i].Elements[j].IsBaseAppModel) {
                                return true;
                            }
                        }
                    }
    ...
    $scope.setComments = function () {
        if ($scope.selectedQuery) {
            var newScope = $scope.$new();
            newScope.comments = $scope.selectedQuery.dictAttributes.sfwDescription;
            newScope.onOKClick = function () {
                $rootScope.EditPropertyValue($scope.selectedQuery.dictAttributes.sfwDescription, $scope.selectedQuery.dictAttributes, "sfwDescription", newScope.comments);
                $scope.updateModelForBaseAppModel();
                newScope.dialog.close();

            };
            var template = `<div class="full-height full-width">
                                <div class="portlet-body">
                                    <textarea style='resize:none' ng-model='comments' class='full-height full-width'/>
                                </div>
                                <div class='form-actions'>
                                    <div class="col-xs-4 fright">
                                        <input type="button" value="OK" class="btn dialog-btn-default" ng-click="onOKClick()" />
                                        <input type="button" value="Cancel" class="btn dialog-btn" ng-click="dialog.close()" />
                                    </div>
                                </div>
                            </div>`;
            newScope.dialog = $rootScope.showDialog(newScope, "Comments", template, {
                width: 600, height: 400, isInlineHtml: true
            });
        }
    };
    ...
    $scope.onDetailClick = function () {
        var newScope = $scope.$new();
        newScope.iblnShowAppEntity = false;
        if ($scope.ProjectType == 1 && $scope.currentfile.FileID !== $scope.objEntity.dictAttributes.ID) {
            newScope.iblnShowAppEntity = true;
        }
        newScope.SelectedEntityDetailsDialogTab = 'Details';
        newScope.BaseAppModel = $scope.objEntity.BaseAppModel;
        newScope.IsBaseAppFile = $scope.IsBaseAppFile;
        ...
        if (!($scope.IsBaseAppFile && $scope.objEntity.dictAttributes.sfwReadOnlyRule === 'None')) {
        ...
        newScope.checkValidEntityInheritence = function () {
            if (newScope.strParentEntity) {
                var lstrCurrentEntity = newScope.objEntityID;
                if (newScope.iblnShowAppEntity) {
                    lstrCurrentEntity = $scope.currentfile.FileID;
                }
                hubMain.server.checkValidParentEntity(lstrCurrentEntity, newScope.strParentEntity).done(function (isValidParentEntity) {
        ...
        newScope.setEntityDetails = function () {
            ...
            if (newScope.IsBaseAppFile && newScope.BaseAppModel && newScope.BaseAppModel.dictAttributes.sfwReadOnlyRule && !newScope.objEntityDetails.sfwReadOnlyRule) {
                $rootScope.EditPropertyValue($scope.objEntity.dictAttributes.sfwReadOnlyRule, $scope.objEntity.dictAttributes, "sfwReadOnlyRule", "None");
            }
            else {
        ...
        //    newScope.entityDetailDialog = $rootScope.showDialog(newScope, "Entity Details", "Entity/views/ReadOnlyEntityDetails.html", {
                ...
                $scope.$evalAsync(function () {
                    ...
                    if ($scope.objAttributes) {
                        angular.forEach($scope.objAttributes.Elements, function (value, key) {
                            if (value.dictAttributes.sfwType === "Column" && value.dictAttributes.sfwValue && value.dictAttributes.sfwValue.toLowerCase() !== "app_json_data") {
                                newScope.TableColumnNames.push(value.dictAttributes.sfwValue.toLowerCase());
                            }
                        });
                    }
    ...
    $scope.GetMappedColumns = function () {
        if ($scope.selectedQuery) {
            ...
            if ($scope.objAttributes) {
                angular.forEach($scope.objAttributes.Elements, function (value, key) {
                    if (value.dictAttributes.sfwType === "Column" && value.dictAttributes.sfwValue && value.dictAttributes.sfwValue.toLowerCase() !== "app_json_data") {
                        $scope.TableColumnNames.push(value.dictAttributes.sfwValue.toLowerCase());
                    }
                });
            }
            ...
            $.connection.hubEntityModel.server.getQuerySchemaFields(strQuery, Param).done(function (data) {
                $scope.$apply(function () {
                    ...
                    function iterator(value) {
                        ...
                        if ($scope.TableColumnNames.indexOf(columnName) == -1 && columnName !== "app_json_data") {
    ...
    $scope.onColumnDataTypeChanged = function (attribute) {
        if (attribute && attribute.dictAttributes) {
            if (attribute.dictAttributes.sfwDataType !== "string") {
                attribute.dictAttributes.sfwEncrypt = "";
            }
            if (attribute.dictAttributes.sfwDataType !== "datetime") {
                attribute.dictAttributes.sfwSaveDateAsUtc = "";
            }
            if (['int', 'long', 'short'].indexOf(attribute.dictAttributes.sfwDataType) === -1) {
    ...
    $scope.beforeAddJsonAttribute = function (newObject) {
        //$scope.$evalAsync(function () {
        if (newObject && newObject.dictAttributes && newObject.dictAttributes.ID && newObject.dictAttributes.sfwDataType) {
            var lstrDataTypePrefix = GetDataTypePrefix(newObject.dictAttributes.sfwDataType.toLowerCase());
            newObject.dictAttributes.sfwValue = lstrDataTypePrefix + newObject.dictAttributes.ID;
        }
        //});
    };
    $scope.addAppJsonColumn = function () {
        if ($scope.checkDbAndDirty("adding new column 'APP_JSON_DATA'")) {
            var confirmString = "A new column 'APP_JSON_DATA' will be added in the entity table if not exists, entity columns will be refreshed and entity will be saved. Do you want to continue?";
            if (confirm(confirmString)) {
                $.connection.hubEntityModel.server.addAppJsonDataColumn($scope.objEntity.dictAttributes.ID).done(function (data) {
                    $scope.$evalAsync(function () {
                        if (data) {
                            //refresh entity columns.
                            $scope.refreshColumns(false);
                        }
                    });
                });
            }
    ...
    $scope.checkHasAppJsonColumn = function () {
        $scope.hasAppJsonDataColumn = $scope.objAttributes.Elements && $scope.objAttributes.Elements.some(function (attr) { return attr.dictAttributes.sfwValue && attr.dictAttributes.sfwValue.toLowerCase() === "app_json_data"; });
    };
    $scope.updateModelForBaseAppModel = function (updateParentName) {
        if ($scope.IsBaseAppFile && $scope.objEntity && $scope.objEntity.BaseAppModel) {
            $scope.$evalAsync(function () {
                updateModelForBaseAppModel($scope.objEntity, $scope.objEntity.BaseAppModel, updateParentName);
            });
        }
    };
    ...
    $scope.onSelectOneToOne = function (selOneToOne) {
        $scope.selOneToOne = selOneToOne;
    };
    $scope.onSelectOneToMany = function (selOneToMany) {
        $scope.selOneToMany = selOneToMany;
    };
    $scope.onSelectProperty = function (selProperty) {
        $scope.selProperty = selProperty;
    };
    $scope.onSelectExpression = function (selExpression) {
        $scope.selExpression = selExpression;
    };
    $scope.canDeleteOneToOne = function () {
        var retValue = true;
        if ($scope.selOneToOne && !($scope.selOneToOne.IsBaseAppModel || $scope.selOneToOne.BaseAppModel)) {
            retValue = false;
        }
        return retValue;
    };
    $scope.canDeleteOneToMany = function () {
        var retValue = true;
        if ($scope.selOneToMany && !($scope.selOneToMany.IsBaseAppModel || $scope.selOneToMany.BaseAppModel)) {
            retValue = false;
        }
        return retValue;
    };
    $scope.canDeleteProperty = function () {
        var retValue = true;
        if ($scope.selProperty && !($scope.selProperty.IsBaseAppModel || $scope.selProperty.BaseAppModel)) {
            retValue = false;
        }
        return retValue;
    };
    $scope.canDeleteExpression = function () {
        var retValue = true;
        if ($scope.selExpression && !($scope.selExpression.IsBaseAppModel || $scope.selExpression.BaseAppModel)) {
            retValue = false;
        }
        return retValue;
    };
    $scope.onSelectConstraint = function (constraint) {
        $scope.selectedConstraint = constraint;
    }
    $scope.canDeleteConstraint = function () {
        var retValue = true;
        if ($scope.selectedConstraint && !($scope.selectedConstraint.IsBaseAppModel || $scope.selectedConstraint.BaseAppModel)) {
            retValue = false;
        }
        return retValue;
    };
        ...
        link: function (scope, element, attrs) {
            ...
            scope.getIntellisenseData = function (entityID) {
                ...
                if (scope.isdelete) {
                    ...
                    filteredData = $Entityintellisenseservice.GetIntellisenseData(entityID, scope.type, scope.datatype, true, true, true, true, false, true);

                    //Remove same xml method from intellisense to avoid recurrsion.
                    if (scope.isxmlmethod && entityID === scope.entityid && scope.xmlMethodObject && scope.xmlMethodObject.dictAttributes && scope.xmlMethodObject.dictAttributes.ID) {
                        filteredData = filteredData.filter(function (itm) { return !(itm.Type && itm.Type === "XMLMethod" && itm.ID === scope.xmlMethodObject.dictAttributes.ID); });
                    }
                }
                return filteredData;
            };
                    ...
                    if (scope.objectfield) {
                        if (scope.objectfield && scope.entityid) {
                            ...
                            if (obj) {
                                ...
                                if (Selectedobj.Type == "Method" || Selectedobj.Type == "Rule" || Selectedobj.Type === "XMLMethod") {
            ...
            scope.clearFields = function () {
                ...
                scope.model.dictAttributes.sfwOrderBy = "";
                scope.model.dictAttributes.sfwSortOrder = "";
                scope.model.dictAttributes.sfwDataType = "";
            };
...
function AddEditConstraintsController(autoSave, attributeName, entityModel, entityId, scope) {
        ...
        if (ctrl.attributeConstraintModel && !ctrl.isNewMode) {
            ...
            if (ctrl.attributeConstraintModel.dictAttributes.sfwMaxValue) {
                ...
                if (ctrl.attributePortalMessages) {
                    ...
                    for (var i = 0, len = portals.length; i < len; i++) {
                        var portalMessage = new PortalMessage(portals[i].dictAttributes.sfwPortalName, portals[i].dictAttributes.sfwMaxMessageId);
            ...
            if (ctrl.ruleQueryConstraints) {
                for (var index = 0, len = ctrl.ruleQueryConstraints.Elements.length; index < len; index++) {
                    ...
                    if (ctrl.ruleQueryConstraints.Elements[index].dictAttributes.sfwRuleQueryParameter) {
                        ruleQueryConstraint.messageParameters = ctrl.ruleQueryConstraints.Elements[index].dictAttributes.sfwRuleQueryParameter;
                    }
                    ...
                    if (ruleQueryPortals) {
                        for (var i = 0, portalLength = ruleQueryPortals.Elements.length; i < portalLength; i++) {
                            ...
                            if (ruleQueryPortals.Elements[i].dictAttributes.sfwRuleQueryParameter) {
                                ruleQueryPortal.messageParameters = ruleQueryPortals.Elements[i].dictAttributes.sfwRuleQueryParameter;
                            }
    ...
    ctrl.onOKClick = function () {
        ...
        for (var index = 0, len = ctrl.lstConstraints.length; index < len; index++) {
            ...
            if (!constraint.messageParameters) {
                constraint.messageParameters = "";
            }
            constraint.portalMessages.forEach(function (itm) {
                if (!itm.messageParameters) {
                    itm.messageParameters = "";
                }
            });
            ...
            //Update Rule/Query Constraint
                ...
                var ruleQueryConstraintModel = new BaseModel("rulequery", {
                    ...
                    sfwRuleQueryParameter: constraint.messageParameters,
                ...
                if (constraint.portalMessages.length) {
                    ...
                    for (var i = 0, portalLength = constraint.portalMessages.length; i < portalLength; i++) {
                        portalMessagesModel.Elements.push(new BaseModel("portal", { sfwPortalName: constraint.portalMessages[i].portalName, sfwMessageId: constraint.portalMessages[i].messageId, sfwRuleQueryParameter: constraint.portalMessages[i].messageParameters }));
                    }
                }
