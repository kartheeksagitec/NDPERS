...
{
    [Serializable]
    public class busHIPAADentalCIGNAFileOut : busFileBaseOut
    {
        ...
        private void LoadAndUpdatesInterchangeControlNo()
        {
            ...
            lcdoCodeValue = busGlobalFunctions.GetCodeValueDetails(busConstant.SystemConstantCodeID, busConstant.SystemConstantISADentalCIGNA);
        ...
        private string GetBenefitOptionCode(busPersonAccountGhdv aobjDental)
        {
            string lstrBOC = string.Empty;
            if (aobjDental.ibusPerson != null)
            {
                if (aobjDental.ibusPerson.ibusPersonCurrentAddress == null)
                    aobjDental.ibusPerson.LoadPersonCurrentAddress();
                if (aobjDental.ibusPerson.ibusPersonCurrentAddress.icdoPersonCurrentAddress != null)
                    lstrBOC = aobjDental.ibusPerson.ibusPersonCurrentAddress.icdoPersonCurrentAddress.lstr_Benefit_Option_Code;
            }
            return lstrBOC;
        }
        ...
        public void LoadDentalCIGNA(DataTable adtbDentalCIGNA)
        {
            ...
            if (Convert.ToString(busGlobalFunctions.GetData1ByCodeValue(52, busConstant.New5010HIPAALayoutForCigna, iobjPassInfo)) == busConstant.Flag_Yes) //pir 8570
        ...
        private string GetHeaderDetails()
        {
            ...
            lstrHeader = "ISA*00*          *00*          *30*450282090      *" + (iblnIsLayout5010 ? "30" : "20")/*pir 8646*/ + "*029053964      *" + ldtTodaysDate.ToString("yyMMdd") + "*" + ldtTodaysDate.ToString("HHmm") + "*" +
                            (iblnIsLayout5010 ? "^" : "U") + "*" + (iblnIsLayout5010 ? "00501" : "00401") + "*" + lstrInterchangeControlNo + "*1*P*>~" + "\r\n";
            lstrHeader += "GS*BE*450282090*029053964*" + ldtTodaysDate.ToString("yyyyMMdd") + "*" + ldtTodaysDate.ToString("HHmm") + "*1*X*" +
            ...
            lstrHeader += "N1*IN*CIGNA*FI*06-0303370~" + "\r\n";
            return lstrHeader;
        }
        ...
        private string GetSubscribersSegments(busPersonAccountGhdv aobjDental)
        {
            ...
            lstrResult = GetINS(true, aobjDental.IsCOBRAValueSelected(), null, aobjDental);
            ...
            lstrResult += GetDTP(true, aobjDental);
            lstrResult += "~\r\n";
        ...
        private string GetDependentsSegments(busPersonAccountDependent aobjPADependent, busPersonAccountGhdv aobjDental)
        {
            ...
            lstrResult += GetHD(aobjDental);
            lstrResult += GetDTP(false, aobjDental, aobjPADependent);
            lstrResult += "~\r\n";
            lstrResult += GetREFMemberPolicy(aobjDental);
            return lstrResult;
        }
        ...
        private string GetINS(bool ablnIsSubscriber, bool ablnIsCOBRA, busPersonAccountDependent aobjPADependent, busPersonAccountGhdv aobjDental)
        {
            ...
            if (ablnIsSubscriber)
            {
                ...
                if (aobjDental.ibusPersonEmploymentDetail.icdoPersonEmploymentDetail.type_value == busConstant.PersonJobTypePermanent)
                    lstrINS += "FT*";
                else if (aobjDental.icdoPersonAccountGhdv.dental_insurance_type_value == busConstant.DentalInsuranceTypeRetiree)
                    lstrINS += "RT*";
                else
                {
                    //pir 8646
        ...
        private string GetHD(busPersonAccountGhdv aobjDental)
        {
            ...
            lstrHD += "DEN**";
            // Coverage Level Code
            if (aobjDental.icdoPersonAccountGhdv.level_of_coverage_value == busConstant.DentalLevelofCoverageFamily)
                lstrHD += "FAM";
            else if (aobjDental.icdoPersonAccountGhdv.level_of_coverage_value == busConstant.DentalLevelofCoverageIndividual)
                lstrHD += "EMP";
            else if (aobjDental.icdoPersonAccountGhdv.level_of_coverage_value == busConstant.DentalLevelofCoverageIndividualChild)
                lstrHD += "ECH";
            else if (aobjDental.icdoPersonAccountGhdv.level_of_coverage_value == busConstant.DentalLevelofCoverageIndividualSpouse)
                lstrHD += "ESP";
        ...
        // DTP - Dental Coverage Dates
        private string GetDTP(bool ablnIsSubscriber, busPersonAccountGhdv abusDental, busPersonAccountDependent aobjPADependent = null)
        {
            _lintTransactionSetCount += 1;
            string lstrDTP = "DTP*";
            if (ablnIsSubscriber)
            {
                if (abusDental.icdoPersonAccount.plan_participation_status_value == busConstant.PlanParticipationStatusInsuranceEnrolled)
                {
                    ...
                    lstrDTP += "D8*";
                    lstrDTP += abusDental.icdoPersonAccount.history_change_date_no_null.ToString(busConstant.DateFormatD8);
                }
                        ...
                        abusDental.icdoPersonAccount.plan_participation_status_value == busConstant.PlanParticipationStatusInsuranceCancelled)  // PROD PIR ID 4242
                {
                    // Date Time Qualifier
                    lstrDTP += "349*";
                    // Date Time Format Qualifier
        ...
        private string GetREFMemberPolicy(busPersonAccountGhdv aobjDental)
        {
            ...
            lstrREF += GetBranch(aobjDental);
            lstrREF += GetBenefitOptionCode(aobjDental);
            if (busGlobalFunctions.IsLastCharacterAsterisk(lstrREF))
                lstrREF = busGlobalFunctions.RemoveLastCharacter(lstrREF);
            lstrREF += "~";
            return lstrREF;
