...
{
    [Serializable]
    public class busHIPAAHealthSanfordFileOut : busFileBaseOut
    {
        ...
        {
            istrFileName = "834.NDPERS.700093." + ldtTodaysDate.ToString(busConstant.DateFormat) + busConstant.FileFormattxt;
        }
        ...
        public Collection<int> iclbHealthSanford { get; set; }
        ...
        public void LoadHealthSanford(DataTable adtbHealthBCBS)
        {
            iclbHealthSanford = (Collection<int>)iarrParameters[0];
        ...
        public override void FinalizeFile()
        {
            ...
            // UAT PIR ID 2170
            //PIR 24703 (Commented the function to Stop creating the annual rptBCBS Report)
            //CreateSanfordReport();
        ...
        public override void BeforeWriteRecord()
        {
            if (iobjDetail is int)
            {
                ...
                if (!lbusHealth.iblnIsActiveMember)//PIR 22945 - If 'Dependent of' is selected then load that members person account.
                    lbusHealth.iblnIsDependentCobra = lbusHealth.ibusPerson.IsDependentCobra(busConstant.PlanIdGroupHealth, lbusHealth.idtPlanEffectiveDate, ref lcdoMemberPersonAccount, lbusHealth.icdoPersonAccount.from_person_account_id);
                //PROD PIR ID 6761  -- End                    
                //lbusHealth.DetermineEnrollmentAndLoadObjects(lbusHealth.idtPlanEffectiveDate, false); //PROD PIR ID 6761

                //PIR 17559 - The latest history's rate_structure_code should be considered. 
                lbusHealth.LoadPersonAccountGHDVHistory();
                //if (lbusHealth.iclbPersonAccountGHDVHistory.IsNotNull())
                //{
                //    lbusHealth.icdoPersonAccountGhdv.rate_structure_code = lbusHealth.iclbPersonAccountGHDVHistory[0].icdoPersonAccountGhdvHistory.rate_structure_code;
                //}

                if (lbusHealth.icdoPersonAccountGhdv.derived_rate_structure_code.IsNullOrEmpty())
                { 
                    if (lbusHealth.icdoPersonAccountGhdv.rate_structure_code.IsNullOrEmpty())
                    {
                ...
                foreach (busPersonAccountDependent lobjPADependent in lbusHealth.iclbPersonAccountDependent)
                {
                    ...
                    //PIR 17395 - is_bcbs_file_sent_flag is not used to select dependent record.
                    // SYS PIR ID 2614                            
        ...
        private string GetHeaderDetails()
        {
            ...
            lstrHeader = "ISA*00*          *00*          *ZZ*ND00913        *30*911842494      *" + ldtTodaysDate.ToString("yyMMdd") + "*" + ldtTodaysDate.ToString("HHmm") + "*" +
                        (iblnIsLayout5010 ? "^" : "U") + "*" + (iblnIsLayout5010 ? "00501" : "00401") + "*" + lstrInterchangeControlNo + "*1*P*:~" + "\r\n";
            lstrHeader += "GS*BE*ND00913*" + (iblnIsLayout5010 ? "Sanford Health Plan*" : "0001*") + ldtTodaysDate.ToString("yyyyMMdd") + "*" + ldtTodaysDate.ToString("HHmm") + "*1*X*" +
        ...
        private string GetSTHeader()
        {
            ...
            lstrSTHeader += "N1*IN*Sanford Health Plan*FI*911842494" + "~\r\n";
            _lintTransactionSetCount += 6;
            return lstrSTHeader;
        }
        ...
        private string GetSubscribersSegments(busPersonAccountGhdv abusHealth)
        {
            ...
            lstrResult += GetREFPerslinkID(abusHealth.ibusPerson.icdoPerson.person_id); //PIR 14179
		...
		//PIR 14179
        private string GetREFPerslinkID(int astrPersonID)
        {
            _lintTransactionSetCount += 1;
            string lstrREF = "REF*"; // Segment Header
            lstrREF += "23*";   // Reference Identification Qualifier
            lstrREF += astrPersonID;    // Reference Identification
            if (busGlobalFunctions.IsLastCharacterAsterisk(lstrREF))
                lstrREF = busGlobalFunctions.RemoveLastCharacter(lstrREF);
            lstrREF += "~\r\n";
            return lstrREF;
        ...
        private string GetNM1(bool ablnIsSubscriber, busPersonAccountDependent aobjPADependent, busPersonAccountGhdv abusHealth)
        {
            ...
            if (ablnIsSubscriber)
            {
                ...
                if (!string.IsNullOrEmpty(abusHealth.ibusPerson.icdoPerson.name_prefix_description))
                    lstrNM1 += "*"; //PIR 16149 Remove Prefix
                else
                    lstrNM1 += "*";
        ...
        private string GetDTP(bool ablnIsSubscriber, busPersonAccountGhdv abusHealth, busPersonAccountDependent aobjPADependent = null)
        {
            ...
            if (ablnIsSubscriber)
            {
                        ...
                        abusHealth.icdoPersonAccount.header_plan_participation_status_value == busConstant.PlanParticipationStatusInsuranceCancelled) // PROD PIR ID 4242
                {
                    ...
                    //PIR 25435
                    else if (abusHealth.icdoPersonAccount.plan_participation_status_value == busConstant.PlanParticipationStatusInsuranceEnrolled 
                        && DateTime.Today.AddDays(31) < abusHealth.icdoPersonAccount.end_date)//PIR 27167 - Changed 30 days to 31
                    {
                        lstrDTP += "348*";                         
                        lstrDTP += "D8*";   
                        lstrDTP += abusHealth.icdoPersonAccount.history_change_date_no_null.ToString(busConstant.DateFormatD8);
                    }
                    else
                    {
                        lstrDTP += "349*";  // Date Time Qualifier                            
        ...
        public void CreateSanfordReport()
        {
