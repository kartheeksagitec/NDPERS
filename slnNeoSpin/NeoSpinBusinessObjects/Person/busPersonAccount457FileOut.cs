...
{
    [Serializable]
    public class busPersonAccount457FileOut : busFileBaseOut
    {
        ...
        public void Load457MembersForTIAACREF(DataTable ldtb457Members)
        {
            // The same query is used in the update query too.
        ...
        public override void BeforeWriteRecord()
        {
            ...
            {
                if (iobjDetail is busPersonAccountDeferredComp)
                {
                    char[] temporary = new char[630];//PIR 9122
                    char[] temp1 = new char[460];
                    StringBuilder temp = new StringBuilder();
                    StringBuilder lsbrHIPAADetail = new StringBuilder();
                    busPersonAccountDeferredComp lobjPersonAccountDeferredComp = (busPersonAccountDeferredComp)iobjDetail;

                    //PIR 9122
                    if ((lobjPersonAccountDeferredComp.ibusPerson.ibusPersonCurrentAddress.icdoPersonAddress.foreign_province.IsNullOrEmpty()))
                    {
                    ...
                    if (!(lobjPersonAccountDeferredComp.ibusPerson.ibusPersonCurrentAddress.icdoPersonAddress.addr_line_2.IsNullOrEmpty()))
                    {
                        lsbrHIPAADetail.Append("405546  " + lobjPersonAccountDeferredComp.ibusPerson.icdoPerson.ssn + "062");
                        if (lobjPersonAccountDeferredComp.ibusPerson.ibusPersonCurrentAddress.icdoPersonAddress.addr_line_2.Length <= 40)
                        {
                            lsbrHIPAADetail.Append(lobjPersonAccountDeferredComp.ibusPerson.ibusPersonCurrentAddress.icdoPersonAddress.addr_line_2.ToUpper());
                            lsbrHIPAADetail.Append(' ', (40 - lobjPersonAccountDeferredComp.ibusPerson.ibusPersonCurrentAddress.icdoPersonAddress.addr_line_2.Length));
                        }
                        else
                        {
                            lsbrHIPAADetail.Append(lobjPersonAccountDeferredComp.ibusPerson.ibusPersonCurrentAddress.icdoPersonAddress.addr_line_2.Substring(0, 40));
                        }
                        ...
                        if (!(lobjPersonAccountDeferredComp.ibusPerson.ibusPersonCurrentAddress.icdoPersonAddress.foreign_province.IsNullOrEmpty()))
                        {
                            ...
                            lsbrHIPAADetail.Append("405546  " + lobjPersonAccountDeferredComp.ibusPerson.icdoPerson.ssn + "063");
                            lsbrHIPAADetail.Append(lobjPersonAccountDeferredComp.ibusPerson.ibusPersonCurrentAddress.icdoPersonAddress.foreign_province.ToUpper()
                                + ", "
                                + lobjPersonAccountDeferredComp.ibusPerson.ibusPersonCurrentAddress.icdoPersonAddress.addr_country_description.ToUpper()
                                + " "
                                + lobjPersonAccountDeferredComp.ibusPerson.ibusPersonCurrentAddress.icdoPersonAddress.foreign_postal_code.ToUpper());
                        }
                        lsbrHIPAADetail.Append(" \r\n");
                    ...
                    else if ((lobjPersonAccountDeferredComp.ibusPerson.ibusPersonCurrentAddress.icdoPersonAddress.addr_line_2.IsNullOrEmpty()))
                    {
        ...
        {
            if (lstrTIAAFlag == busConstant.Flag_Yes)
                istrFileName = "405546" + ldteNextRunDate.ToString(busConstant.DateFormatD8) + "A" + busConstant.FileFormattxt;
        }
